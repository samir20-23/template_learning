...
@push('scripts')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
                    ...
                    datalabels: {
                        ...
                        formatter: function(value, context) {
                            if (context.chart.config.type === 'pie' || context.chart.config.type ===
                                'doughnut' || context.chart.config.type === 'polarArea') {
                                    ...
                                    meta.data.forEach(function(element, index) {
                                        ...
                                        if (element.options) {
                                            if (chartInstance.config.type === 'bar') {
                                                element.options.borderWidth = animation
                                                    .currentStep / animation.numSteps * 2;
                                            } else if (chartInstance.config.type ===
                                                'line') {
                                                element.options.radius = animation
                                                    .currentStep / animation.numSteps * 6;
                                            }
                                        }
                                    });
                                }
            ...
            const months = monthlyGrowth.length ? monthlyGrowth.map(item => item.month) : ['Jan', 'Feb', 'Mar',
                'Apr', 'May', 'Jun'
            ];
            ...
            // xx
            const docTypeLabels = documentTypes.length ?
                documentTypes.slice(0, 10).map(item => item.type) : ['PDF Documents', 'Word Documents',
                    'Excel Files', 'PowerPoint'
                ];

            const docTypeData = documentTypes.length ?
                documentTypes.slice(0, 10).map(item => item.count) : [
            ...
            const trendMonths = validationTrends.length ? validationTrends.map(item => item.month) : ['Jan', 'Feb',
                'Mar', 'Apr', 'May', 'Jun'
            ];
            ...
            const charts = [{
                    chart: barChart,
                    id: 'barChart',
                    title: 'Documents and Categories Overview'
                },
                {
                    chart: lineChart,
                    id: 'lineChart',
                ...
                {
                    chart: validationStatusChart,
                    id: 'validationStatusChart',
                    title: 'Validation Status Distribution'
                },
                {
                    chart: validationTrendChart,
                    id: 'validationTrendChart',
            ...
            function updateChartsTheme() {
                charts.forEach(({
                    ...
                    if (chart.config.type === 'bar' || chart.config.type === 'line') {
                        ...
                        chart.data.datasets[0].backgroundColor = colorKeys.map(key => colors[key].base +
                            '80').slice(0, chart.data.labels.length);
                        chart.data.datasets[0].borderColor = colorKeys.map(key => colors[key].base).slice(0,
                    ...
                    if (chart.options.scales) {
                        Object.keys(chart.options.scales).forEach(scaleKey => {
                            ...
                            if (scale.ticks) {
                                scale.ticks.color = scaleKey === 'y1' ? colors.primary.base : colors
                                    .text;
                            }
                            if (scale.grid) {
                                scale.grid.color = scaleKey === 'x' ? colors.grid + '40' : colors
                                    .grid;
                            }
                            ...
                            if (scale.title) {
                                scale.title.color = scaleKey === 'y1' ? colors.primary.base : colors
                                    .text;
                            }
                        });
                    }
