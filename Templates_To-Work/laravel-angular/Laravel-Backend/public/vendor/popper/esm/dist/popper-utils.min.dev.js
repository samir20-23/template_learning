...
function p(a, b, c, d) {
  return Math.max(b['offset' + a], b['scroll' + a], c['client' + a], c['offset' + a], c['scroll' + a], h(10) ? parseInt(c['offset' + a]) + parseInt(d['margin' + ('Height' === a ? 'Top' : 'Left')]) + parseInt(d['margin' + ('Height' === a ? 'Bottom' : 'Right')]) : 0);
}
...
function y(a, e, f, g) {
  ...
  if ('viewport' === g) i = v(j, h);else {
    var k;
    'scrollParent' === g ? (k = c(b(e)), 'BODY' === k.nodeName && (k = a.ownerDocument.documentElement)) : 'window' === g ? k = a.ownerDocument.documentElement : k = g;
    var m = u(k, j, h);
    ...
    if ('HTML' === k.nodeName && !w(j)) {
      var n = q(a.ownerDocument),
          o = n.height,
          p = n.width;
      i.top += m.top - m.marginTop, i.bottom = o + m.top, i.left += m.left - m.marginLeft, i.right = p + m.left;
    } else i = m;
  }
  f = f || 0;
  var r = 'number' == typeof f;
  return i.left += r ? f : f.left || 0, i.top += r ? f : f.top || 0, i.right -= r ? f : f.right || 0, i.bottom -= r ? f : f.bottom || 0, i;
}
